...
- `intended_consumers`: consumer types expected to use the profile, such as `human`, `cursor`, `claude`, `codex`, or `ci-governance`
- `context_quality_max_active_items`: optional warning threshold for the total active context selected by the profile

Profiles that intentionally carry broad context should set `context_quality_max_active_items` explicitly. v1 surfaces this rule in AGENTS.md, context output, audit, and doctor warnings; it does not enforce runtime orchestration or permission policy.
