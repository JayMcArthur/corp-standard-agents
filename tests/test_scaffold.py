...
class ScaffoldTests(unittest.TestCase):
    ...
    def test_init_corp_repo_writes_root_docs_examples_and_users_readme(self) -> None:
        ...
        self.assertTrue((corp / "org" / "contexts" / "authoring-example" / "item.toml").exists())
        self.assertTrue((corp / "org" / "completion_gates" / "definition-of-done" / "item.toml").exists())
        self.assertTrue((corp / "org" / "playbooks" / "coder-loop" / "item.toml").exists())
        self.assertTrue((corp / "org" / "playbooks" / "pr-review" / "item.toml").exists())
        self.assertTrue((corp / "org" / "playbooks" / "prep-before-code" / "item.toml").exists())
        self.assertTrue((corp / "org" / "completion_gates" / "prep-artifacts" / "item.toml").exists())
        ...
        self.assertTrue((corp / "repos" / "example-repo" / "completion_gates" / "repo-bootstrap" / "item.toml").exists())
        ...
        completion_gate = (corp / "org" / "completion_gates" / "definition-of-done" / "item.toml").read_text(encoding="utf-8")
        self.assertIn("evidence_required", completion_gate)
    ...
    def test_generated_corp_and_local_user_scaffolds_load_and_resolve(self) -> None:
        ...
        self.assertIn("corp.example-org.completion_gate.definition-of-done", result.active_completion_gates)
        self.assertIn("corp.example-org.completion_gate.prep-artifacts", result.active_completion_gates)
        self.assertIn("corp.example-org.playbook.prep-before-code", result.active_playbooks)
